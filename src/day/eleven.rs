...
fn do_swap_part_one(seats: &Life, i: usize, j: usize) -> bool {
    ...
    match seats[i][j] {
        'L' => neighbours.all(|&seat| seat != '#'),
        '#' => neighbours.filter(|&&seat| seat == '#').count() >= 4,
        _ => false,
    }
}
...
fn do_swap_part_two(seats: &Life, i: usize, j: usize) -> bool {
    ...
    match seats[i][j] {
        'L' => neighbours.all(|seat| seat != '#'),
        '#' => neighbours.filter(|&seat| seat == '#').count() >= 5,
        _ => false,
    }
}

fn play<F: Fn(&Life, usize, usize) -> bool>(seats: &Life, do_swap: F) -> usize {
...
impl Day for Eleven {
    ...
    fn part_one(seats: &Self::Input) -> Self::Output {
        play(&seats, do_swap_part_one)
    }

    fn part_two(seats: &Self::Input) -> Self::Output {
        play(&seats, do_swap_part_two)
    }
